...
#define M_PI 3.14159265358979310000
    ...
    // the hal ide solution
Halide::Func halide_blur(Halide::Func in) {
    Halide::Func bh, bv;
    Halide::Var x, y, xo, yo, xi, yi;
    // The algorithm
    // Halide representation, the algorithm only deùnes the
    // value of each function at any point;
    bh(x, y) = (in(x - 1, y) + in(x, y) + in(x + 1, y)) / 3;
    bv(x, y) = (bh(x, y - 1) + bh(x, y) + bh(x, y + 1)) / 3;
    // The schedule
    // a schedule speciùes species the organization of computation
    // over all points in all functions.
...
int main(int argc, char** argv) {
    ...
    printf("Success! - Antonio Code Halide lesson_01\n");
